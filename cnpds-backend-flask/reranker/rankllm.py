...
class RankLLM(ABC):
    ...
    @abstractmethod
    def run_llm(self, messages: Union[str, List[Dict[str, str]]]) -> Tuple[str, int]:
        """
        Abstract method to run the target language model with a passed in prompt.

        Args:
            messages (Union[str, List[Dict[str, str]]]): The prompt to be processed by the model.
    ...
    @abstractmethod
    def get_num_tokens(self, messages: Union[str, List[Dict[str, str]]]) -> int:
        """
        Abstract method to calculate the number of tokens contained in the given prompt.

        Args:
            messages (Union[str, List[Dict[str, str]]]): The prompt for which to compute the token count for.
    ...
    def permutation_pipeline(
        ...
        """
        messages, in_token_count = self.create_prompt(result, rank_start, rank_end)
        if logging:
            print(f"prompt: {messages}\n")
        permutation, out_token_count = self.run_llm(
            messages, current_window_size=rank_end - rank_start
        )
        if logging:
            print(f"output: {permutation}")
        ranking_exec_info = RankingExecInfo(
            messages, permutation, in_token_count, out_token_count
        )
