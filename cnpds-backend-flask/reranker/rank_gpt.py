...
class Zhipuai(RankLLM):
    ...
    def run_llm(
        ...
        response, total_token = self._call_completion(
            messages=prompt,
            temperature=0.01,
            completion_mode=Zhipuai.CompletionMode.CHAT,
            return_text=True,
            model=self._model,
        )
    ...
    def create_rank_gpt_prompt(
        ...
        # 由于glm4有128k的context size 不限制了
        
        max_length = 300 * (self._window_size / (rank_end - rank_start))
        while True:
            messages = self._get_prefix_for_rank_gpt_prompt(query, num)
            rank = 0
            for cand in result.hits[rank_start:rank_end]:
                rank += 1
                content = cand['content']
                messages.append(
                    {
                        "role": "user",
                        "content": f"[{rank}] {fix_text(self._replace_number(content))}",
                    }
                )
                messages.append(
